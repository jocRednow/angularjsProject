app.controller("kioskFormCtrl", function kioskFormCtrl($scope, $rootScope, $window, $timeout, $http, APIService) {
    ...
    $scope.sendKioskFormData = function () {
        APIService.postSpecValidForm($scope.formData).then(function (response) {
            if (response.status == 200) {
                APIService.postSpecKioskFormData($scope.formData.id).then(function (response) {
                    if (response.status == 200) {
                        console.log("SEND form kiosk.");

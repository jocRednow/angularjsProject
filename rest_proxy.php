 ...
 */
require_once($_SERVER['DOCUMENT_ROOT'].'/crypto.php');
require_once($_SERVER['DOCUMENT_ROOT'].'/conf/Tomcat_Conf.php');

use Nodus_Configs as Conf;

$valid_requests = array(
	Conf\Tomcat_Conf::TOMCAT_IP
);
...
$callerId = nodus_decrypt($_COOKIE["nodus_token"]);
if ($callerId)
	$request_headers[] = "X-Caller: ".$callerId;
rest_proxy_log("caller", $callerId);
...
if ( isset( $_REQUEST['csurl'] ) ) {
    ...
    $request_url = $_SERVER['HTTP_X_PROXY_URL']; //urldecode( $_SERVER['HTTP_X_PROXY_URL'] ); // FIX
} else {
...
//$f = fopen(elgg_get_root_path().'rest_proxy_log.txt', 'c');
...
if ( /*strpos($request_url, "saveGrade") &&*/ mb_strpos($response, "HTTP/1.1 100 Continue") === 0){
	list($temp, $response_headers, $response_content) = explode("\r\n\r\n", $response, 3);
}
else {
...
foreach ( $response_headers as $key => $response_header ) {
	...
	if ( !preg_match( '/^(Transfer-Encoding):/', $response_header ) && mb_strpos($response_header, 'X-Caller') === false ) { 
...
function rest_proxy_log($name, $val){
		$file = $_SERVER['DOCUMENT_ROOT']."/rest_proxy.log";
		if ($val){
			$msg = $name.": ".$val."\n";
